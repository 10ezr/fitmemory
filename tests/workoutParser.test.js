...
describe('WorkoutParser', () => {
  ...
  describe('isWorkoutMessage', () => {
    test('should identify workout messages', () => {
      expect(WorkoutParser.isWorkoutMessage('I did 3x10 squats 80kg today')).toBe(true);
      expect(WorkoutParser.isWorkoutMessage('Completed legs workout')).toBe(true);
      expect(WorkoutParser.isWorkoutMessage('Ran 5km this morning')).toBe(true);
      expect(WorkoutParser.isWorkoutMessage('finished bench press session')).toBe(true);
    });
    ...
    test('should not identify non-workout messages', () => {
      ...
      expect(WorkoutParser.isWorkoutMessage('Tell me about nutrition')).toBe(false);
    });
  });
  ...
  describe('parseWorkout', () => {
    ...
    test('should parse multiple exercises', () => {
      const result = WorkoutParser.parseWorkout('Did legs today - 3x8 squats 100kg, 3x12 lunges');
    ...
    test('should handle workout with duration', () => {
      const result = WorkoutParser.parseWorkout('20 minutes cardio on treadmill');
    ...
    test('should handle date variations', () => {
      ...
      const resultYesterday = WorkoutParser.parseWorkout('Did squats yesterday');
      
      expect(resultToday.date).toBeTruthy();
      expect(resultYesterday.date).toBeTruthy();
      expect(resultYesterday.date.getTime()).toBeLessThan(resultToday.date.getTime());
    });
  });
  ...
  describe('cleanExerciseName', () => {
    test('should clean and normalize exercise names', () => {
      ...
      expect(WorkoutParser.cleanExerciseName('Bench-Press!!')).toBe('bench press');
