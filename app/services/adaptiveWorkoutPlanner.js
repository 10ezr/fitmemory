...
class AdaptiveWorkoutPlanner {
  ...
  analyzeProgressTrend(workouts) {
    ...
    const volumes = recentWorkouts.map(
      (w) =>
  ...
  identifyAdaptations(workouts, feedback, analysis) {
    ...
    if (analysis.consistencyScore < 50) {
      adaptations.push({
        ...
        reason:
          "Low consistency - recommend fewer but more achievable sessions",
      });
  ...
  generateRecommendations(analysis) {
    ...
    if (analysis.consistencyScore < 70) {
      recommendations.push({
        ...
        description:
          "Try scheduling shorter, more manageable sessions at the same time each day.",
    ...
    if (analysis.progressTrend === "stable") {
      recommendations.push({
        ...
        description:
          "Your performance has plateaued. Consider increasing intensity or trying new exercises.",
    ...
    if (analysis.performanceMetrics.exerciseVariety < 6) {
      recommendations.push({
        ...
        description:
          "Adding variety prevents boredom and works different muscle groups.",
  ...
  selectExercises(options) {
    ...
    let availableExercises = Object.entries(exerciseDatabase)
      .filter(([key, exercise]) => {
        ...
        if (exercise.equipment !== equipment && equipment !== "any") {
          if (equipment === "none" && exercise.equipment !== "none")
            return false;
          if (equipment !== "none" && exercise.equipment === "none")
            return false;
        }
    ...
    if (focus !== "general") {
      availableExercises = availableExercises.filter(
        (ex) => ex.category === focus || ex.type === focus
      );
    }

    // Avoid recent exercises (add variety)
    const recentExerciseNames = recentExercises.map((ex) => ex.name);
    const freshExercises = availableExercises.filter(
      (ex) => !recentExerciseNames.includes(ex.name)
    );

    // Select exercises based on duration
    const targetExerciseCount = this.calculateExerciseCount(
      duration,
      difficulty
    );
    ...
    const selectedPool =
      freshExercises.length >= targetExerciseCount
        ? freshExercises
        : availableExercises;

    // Select balanced mix
    return this.selectBalancedExercises(
      selectedPool,
      targetExerciseCount,
      focus
    );
  ...
  calculateExerciseCount(duration, difficulty) {
    ...
    return Math.max(
      3,
      Math.min(8, Math.floor(baseCount * difficultyMultiplier[difficulty]))
    );
  }
  ...
  generateProgressionNotes(analysis) {
    ...
    if (analysis.progressTrend === "improving") {
      notes.push(
        "🎯 You're progressing well! Consider increasing reps by 2-3 next session."
      );
    }

    if (analysis.consistencyScore > 80) {
      notes.push(
        "🔥 Great consistency! You're ready for more challenging variations."
      );
    }

    if (analysis.performanceMetrics.exerciseVariety < 5) {
      notes.push(
        "🔄 Try to incorporate 1-2 new exercises to keep progressing."
      );
    }
  ...
  recordFeedback(workoutId, feedback) {
    ...
    const existingFeedback = JSON.parse(
      localStorage.getItem("workoutFeedback") || "[]"
    );
  ...
  adaptProfileFromFeedback(feedback) {
    ...
    feedback.exercises?.forEach((exerciseFeedback) => {
      if (exerciseFeedback.enjoyment >= 4) {
        if (
          !this.userProfile.preferences.favoriteExercises.includes(
            exerciseFeedback.name
          )
        ) {
          this.userProfile.preferences.favoriteExercises.push(
            exerciseFeedback.name
          );
        }
      } else if (exerciseFeedback.enjoyment <= 2) {
        if (
          !this.userProfile.preferences.excludeExercises.includes(
            exerciseFeedback.name
          )
        ) {
          this.userProfile.preferences.excludeExercises.push(
            exerciseFeedback.name
          );
        }
      }
