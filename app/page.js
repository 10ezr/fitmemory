...
function ProgressRing({ progress = 0, size = 92, strokeWidth = 8, color = "#7c3aed" }) {
  ...
  return (
    <div className="relative">
      <svg width={size} height={size} className="-rotate-90 drop-shadow-[0_0_8px_rgba(124,58,237,.2)]">
        <circle cx={size/2} cy={size/2} r={r} stroke="currentColor" strokeWidth={strokeWidth} fill="none" className="text-muted opacity-25" />
        <circle cx={size/2} cy={size/2} r={r} stroke={color} strokeWidth={strokeWidth} fill="none" strokeDasharray={C} strokeDashoffset={offset} strokeLinecap="round" className="transition-all duration-1000" />
      </svg>
    </div>
  );
}
...
function HydrationWidget() {
  ...
  return (
    <Card className="hover:shadow-sm transition-all cursor-pointer" onClick={() => setGlasses(g => Math.min(goal, g + 1))}>
      <CardContent className="p-4 flex items-center justify-between">
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 bg-cyan-500/20 rounded-full animate-pulse" style={{ opacity: progress / 100 }} />
        </div>
      </CardContent>
    </Card>
  );
...
function RightRail({ stats, sleep, workouts }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      {/* Live reminders */}
      <Card className="hover:shadow-sm transition-all">
        <CardHeader className="pb-2"><CardTitle className="text-sm">Live reminders</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          <div className="flex items-center justify-between"><span>Hydrate</span><Badge variant="secondary">250ml</Badge></div>
          <div className="flex items-center justify-between"><span>Stand</span><Badge variant="secondary">2min</Badge></div>
      ...
      {/* Today timeline */}
      <Card className="hover:shadow-sm transition-all">
        <CardHeader className="pb-2"><CardTitle className="text-sm">Today timeline</CardTitle></CardHeader>
      ...
      <Card className="hover:shadow-sm transition-all">
        <CardHeader className="pb-2"><CardTitle className="text-sm">Quick notes</CardTitle></CardHeader>
        <CardContent>
...
function QuickChat() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setLog(l => [...l, { role: "system", content: "Connection failed. Try again." }]);
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        <div className="max-w-[1400px] mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
            ...
            <Card className="hover:shadow-lg transition-all">
              <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Target className="h-5 w-5" /> Daily targets</CardTitle></CardHeader>
