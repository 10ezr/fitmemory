...
export default function Home() {
  ...
  const sendMessage = async (messageText) => {
    ...
    try {
      ...
      const response = await fetch("/api/converse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: messageText, timestamp: currentTime }) })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || "Failed to send message")

      const assistantMessage = {
        id: (Date.now() + 1).toString(), role: "assistant", content: data.reply, createdAt: new Date().toISOString(),
        workoutLogged: data.workoutLogged, sleepLogged: data.sleepLogged, memoryStored: data.memoryStored,
        workout: data.workout, sleepSession: data.sleepSession, streakUpdate: data.streakUpdate
      }
      ...
      if (data.workoutLogged) showActionToast({ type: 'workout_logged', exercises: data.workout?.exercises })
      if (data.streakUpdate) showActionToast({ type: 'streak_incremented', currentStreak: data.streakUpdate.currentStreak })
      ...
      if (data.workoutLogged || (data.sleepLogged && data.sleepSession)) {
        ...
        if (data.workout && notificationService) {
          notificationService.workoutCompleted({ totalDuration: data.workout.duration || 0, exercises: data.workout.exercises || [] })
        }
      }
    } catch (error) {
    ...
    <>
      <SidebarProvider defaultOpen={true}>
        <div className="flex h-screen w-full overflow-hidden bg-background">
          <FitnessSidebar stats={stats} onDataChange={refreshStatsAndBroadcast} onShowAnalytics={() => setShowAnalytics(true)} />
          <SidebarInset className="flex-1">
            <div className="flex h-full flex-col overflow-hidden">
              <div className="flex-1 overflow-y-auto scrollbar-hide">
                <div className="w-full max-w-4xl mx-auto space-y-4 px-4 py-6">
                  {messages.length === 0 && initialLoading && (<SkeletonChatList count={3} />)}
                  {messages.length === 0 && !initialLoading && (
                    <div className="text-center py-12">
                      ...
                      <p className="text-muted-foreground mb-6 max-w-md mx-auto">I'm your AI health coach. I can help you track workouts, monitor sleep, create plans, and stay motivated on your wellness journey.</p>
              ...
              <div className="bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                <div className="w-full max-w-4xl mx-auto p-4">
                  {messages.length > 0 && (<div className="mb-3"><QuickShortcuts onSelectShortcut={insertQuickMessage} /></div>)}
                    ...
                    <Card className="border-2 border-border/50 focus-within:border-primary/50 focus-within:bg-card transition-all duration-200">
                      <CardContent className="p-3">
                        <div className="flex items-end gap-3">
                          <Textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about fitness, sleep, recovery, or your health goals..." className="flex-1 min-h-[40px] max-h-[120px] resize-none border-0 bg-transparent p-0 focus-visible:ring-0 placeholder:text-muted-foreground/70" rows={1} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSubmit(e) } }} onInput={(e) => { e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 120) + "px" }} />
                          <Button type="submit" disabled={loading || !input.trim()} size="sm" className="shrink-0 rounded-xl px-3 h-9">
                            {loading ? (<div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />) : (<PaperAirplaneIcon className="w-4 h-4" />)}
                          </Button>
                        </div>
          ...
          {showAnalytics && (
            <AnalyticsDashboard onClose={() => setShowAnalytics(false)} workoutData={stats?.recentWorkouts || []} streakData={stats?.streakHistory || []} timerData={timerData} />
          )}
        </div>
      </SidebarProvider>
      <BottomNav />
