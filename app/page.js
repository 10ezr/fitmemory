  ...
  const loadInitialData = async () => {
    try {
      ...
      if (todaysWorkoutRes.ok)
        setTodaysWorkout((await todaysWorkoutRes.json()).workout);
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          setStats(retryFresh);
          realTimeSync.broadcastDataChange(
            "stats",
            retryFresh,
            "page-refresh-retry"
          );
  ...
  const sendMessage = async (messageText) => {
    ...
    try {
      ...
      if (data.memoryStored) {
        console.log(
          `💭 Memory stored: ${
            data.memoryStored.type
          } - ${data.memoryStored.content.substring(0, 50)}...`
        );
      }
      ...
      if (data.streakStatus && notificationService) {
        if (data.streakStatus.streakReset) {
          ...
          data.streakStatus.streakMaintained &&
          data.streakStatus.missedWorkouts >= 2
        ) {
                  ...
                  {messages.length === 0 && !initialLoading && (
                    <div className="text-center py-12">
                      ...
                      <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                        ...
                        workouts, monitor sleep, create plans, and stay
