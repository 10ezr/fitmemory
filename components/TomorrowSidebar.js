...
export default function TomorrowSidebar() {
  ...
  const sourceInfo = useMemo(() => {
    ...
    const sourceMap = {
      'ai-generated': { text: 'Fresh AI', variant: 'default', color: 'text-green-600' },
      'cache': { text: 'Cached', variant: 'secondary', color: 'text-blue-600' },
      'context-stable': { text: 'Stable', variant: 'secondary', color: 'text-gray-600' },
      'rate-limited': { text: 'Rate Limited', variant: 'destructive', color: 'text-orange-600' },
      'throttled': { text: 'Throttled', variant: 'outline', color: 'text-yellow-600' },
      'error-fallback': { text: 'Fallback', variant: 'destructive', color: 'text-red-600' }
    };
    
    return sourceMap[data.source] || { text: data.source, variant: 'outline', color: 'text-gray-600' };
  }, [data?.source]);
                  ...
                  <div className="space-y-2">
                    {(data.workout.exercises || []).map((ex, i) => (
                      ...
                      >
                        ...
                        <Badge variant="outline" className="text-xs font-mono bg-neutral-800/50">
                          {ex.sets}×{ex.reps}
                        </Badge>
                      </div>
                    ))}
