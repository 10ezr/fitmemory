...
function msUntilNextDay(d = new Date()) { const t = new Date(d); t.setHours(24, 0, 0, 0); return Math.max(0, t.getTime() - d.getTime()); }
function msUntil24hFrom(lastMs) { if (!lastMs) return 0; const now = Date.now(); const target = lastMs + 24 * 60 * 60 * 1000; return Math.max(0, target - now); }

export default function FitnessSidebar({ stats, onDataChange }) { 
  ...
  const untilTomorrow = { hours: Math.floor(untilTomorrowMs / 3600000), minutes: Math.floor((untilTomorrowMs % 3600000) / 60000), seconds: Math.floor((untilTomorrowMs % 60000) / 1000) }; 

  const lastSuccessAt = Number(current.lastSuccessAt || 0); 
  const until24hMs = msUntil24hFrom(lastSuccessAt); 
  const until24h = { hours: Math.floor(until24hMs / 3600000), minutes: Math.floor((until24hMs % 3600000) / 60000), seconds: Math.floor((until24hMs % 60000) / 1000) }; 

  const timeFmt = new Intl.DateTimeFormat("en-US", { hour12: true, hour: "numeric", minute: "2-digit", second: "2-digit", timeZone: "Asia/Kolkata" }); 
  const dateFmt = new Intl.DateTimeFormat("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric", timeZone: "Asia/Kolkata" }); 
                  ...
                  <CardContent className="pt-0 space-y-4"> 
                    ...
                    <div className="border-t pt-4"> 
                      ...
                      <div className="grid grid-cols-3 gap-2"> 
                        {[{ value: untilTomorrow.hours }, { value: untilTomorrow.minutes }, { value: untilTomorrow.seconds }].map((b, i) => ( 
                    ...
                    {lastSuccessAt > 0 && ( 
                      <div className="border-t pt-4"> 
                        ...
                        <div className="grid grid-cols-3 gap-2"> 
                          {[{ value: until24h.hours }, { value: until24h.minutes }, { value: until24h.seconds }].map((b, i) => ( 
                            <div key={i} className="bg-card/50 border rounded-lg p-3 text-center"> 
                              <div className="text-xl font-bold text-primary font-mono"> 
                                {String(b.value).padStart(2, "0")} 
                              </div> 
