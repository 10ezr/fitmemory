...
class MemoryService {
  ...
  async getLongTermMemories(query = null, limit = 5, threshold = 0.6) {
    try {
      ...
      const similarMemories = memories
        .map((memory) => ({
          ...memory.toObject(),
          similarity: GeminiService.cosineSimilarity(
            queryEmbedding,
            memory.embedding
          ),
        }))
  ...
  async updateMemory(memoryId, updates) {
    try {
      ...
      if (updates.content) {
        updates.embedding = await this.geminiService.generateEmbedding(
          updates.content
        );
      }

      const memory = await Memory.findByIdAndUpdate(memoryId, updates, {
        new: true,
      });
  ...
  async processRememberCommand(userMessage, context = {}) {
    try {
      ...
      if (contentToRemember && contentToRemember.length > 5) {
        ...
        // Store the memory
        const memory = await this.addMemory(memoryType, contentToRemember, {
          confidence: 0.9,
          userRequested: true,
          timestamp: new Date(),
      ...
      return {
        success: false,
        response:
          "I didn't catch what you wanted me to remember. Could you be more specific?",
      };
    } catch (error) {
  ...
  determineMemoryType(content) {
    ...
    if (
      ...
      lowerContent.includes("tends to")
    ) {
      return "pattern";
    }
    if (
      lowerContent.includes("learned") ||
      lowerContent.includes("noticed") ||
      lowerContent.includes("realized") ||
    ...
    if (
      ...
      lowerContent.includes("accomplished") ||
  ...
  async searchUserMemories(query, options = {}) {
    ...
    try {
      ...
      if (query && query.trim()) {
        const memories = await this.getLongTermMemories(
          query,
          limit * 2,
          threshold
        );
        return memories
          .filter((memory) => !types || types.includes(memory.type))
          .slice(0, limit);
      }
    ...
    for (const action of actions) {
      try {
        switch (action.action) {
          case "memory_add":
            await this.addMemory(action.type || "insight", action.content, {
  ...
  async storeWorkoutPlan(action) {
    try {
      const planContent = `Workout Plan: ${action.exercises
        .map(
          (e) =>
      ...
      await this.addMemory("pattern", planContent, {
        confidence: 0.8,
        type: "workout_plan",
  ...
  async trackProgress(action) {
    try {
      ...
      await this.addMemory("insight", progressContent, {
        confidence: 0.9,
  ...
  async celebrateStreak(action) {
    try {
      ...
      await this.addMemory("achievement", celebrationContent, {
        confidence: 1.0,
    ...
    try {
      ...
      if (existingMemories.length > 0) {
        ...
        await this.updateMemory(existing._id, {
          meta: {
            ...existing.meta,
            confidence: Math.max(existing.meta?.confidence || 0, confidence),
          },
        });
      } else {
        // Create new memory
        await this.addMemory("pattern", content, {
          confidence,
          confirmed: true,
        });
      }
  ...
  async getMemoriesByType(type, limit = 10) {
    try {
      return await Memory.find({ type }).sort({ createdAt: -1 }).limit(limit);
    } catch (error) {
