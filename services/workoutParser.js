...
class WorkoutParser {
  static workoutKeywords = [
    'did', 'completed', 'finished', 'workout', 'trained', 'session',
    'squats', 'deadlifts', 'bench', 'press', 'curls', 'rows', 'pullups',
    'pushups', 'lunges', 'dips', 'chinups', 'overhead', 'lateral',
  ...
  static isWorkoutMessage(message) {
    ...
    return this.workoutKeywords.some(keyword => lowerMessage.includes(keyword)) ||
  ...
  static parseWorkout(message) {
    ...
    const dateMatches = message.match(/(?:yesterday|today|(\d{1,2}[\/\-]\d{1,2}))/i);
    if (dateMatches) {
  ...
  static extractExerciseFromMatch(match, pattern) {
    ...
    const exercise = { name: '', sets: null, reps: null, weightKg: null, durationS: null };
  ...
  static extractWorkoutName(message) {
    ...
    const mentionedParts = bodyParts.filter(part => lowerMessage.includes(part));
    ...
    if (lowerMessage.includes('strength') || lowerMessage.includes('lifting')) return 'Strength Training';
  ...
  static deduplicateExercises(exercises) {
    ...
    exercises.forEach(exercise => {
      ...
      if (seen.has(key)) {
        ...
        existing.sets = Math.max(existing.sets || 0, exercise.sets || 0) || null;
        existing.reps = Math.max(existing.reps || 0, exercise.reps || 0) || null;
        existing.weightKg = Math.max(existing.weightKg || 0, exercise.weightKg || 0) || null;
        existing.durationS = Math.max(existing.durationS || 0, exercise.durationS || 0) || null;
      } else {
